...
object CRDTActorR {
  ...
  // Key-Value Ops
  case class AtomicPut(key: String, value: Int, from: ActorRef[Command]) extends Command
...
class CRDTActorR(
  ...
  override def onMessage(msg: Command): Behavior[Command] = msg match
    ...
    case Put(key, value, from) =>
      ...
      // Simulate a slow operation (random sleep time between 0 and 500ms)
//      Thread.sleep((Math.random() * 1000).toLong)
      crdtstate = crdtstate.put(selfNode, key, value)
      ctx.log.info(s"CRDTActor-$id: CRDT state: $crdtstate")
      // Before broadcast, sleep for a random time between 0 and 500ms
//      Thread.sleep((Math.random() * 1000).toLong)
      // Broadcast the delta to all other actors and reset the delta
      ...
      // AtomicPut is a special case of Put, where the value is always the same
      // and the operation is idempotent
    case AtomicPut(key, value, from) =>
      ctx.log.info(s"CRDTActor-$id: Consuming operation $key -> $value")
      crdtstate = crdtstate.put(selfNode, key, value)
      ctx.log.info(s"CRDTActor-$id: CRDT state: $crdtstate")
      broadcastAndResetDeltas()
